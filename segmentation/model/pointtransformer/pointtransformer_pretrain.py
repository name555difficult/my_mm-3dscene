...
def singleton(cache_key):
    def inner_fn(
        fn,
    ):  #  The inner function takes another function as an argument and returns a wrapper function
        @wraps(fn)
...
def cal_loss(c1, c2, c3, c4, c5, target_points, o_tar):
    ...
    for i in range(o_tar.size(0)):
        tar = (
            target_points[: o_tar[i], :]
            if i == 0
            else target_points[o_tar[i - 1] : o_tar[i], :]
        )
...
class Fold(nn.Module):
    ...
    def forward(self, x):
        ...
        npos = x.size(0)
        features = x.view(npos, self.in_channel, 1).expand(
            npos, self.in_channel, num_sample
        )  # [npos,C,num_sample]
        seed = (
            self.folding_seed.view(1, 2, num_sample)
            .expand(npos, 2, num_sample)
            .to(x.device)
        )  # [npos,2, num_sample]
...
class PointTransformer_model(nn.Module):
    def __init__(self, block, blocks, c=6, k=13):
        ...
        self.dec5 = self._make_dec(
            block, planes[4], 2, share_planes, nsample=nsample[4], is_head=True
        )  # transform p5
        self.dec4 = self._make_dec(
            block, planes[3], 2, share_planes, nsample=nsample[3]
        )  # fusion p5 and p4
        self.dec3 = self._make_dec(
            block, planes[2], 2, share_planes, nsample=nsample[2]
        )  # fusion p4 and p3
        self.dec2 = self._make_dec(
            block, planes[1], 2, share_planes, nsample=nsample[1]
        )  # fusion p3 and p2
...
class PointTransformerSeg(nn.Module):
    def __init__(self, block, blocks, c=6, k=13, moving_average_decay=0.99):
        ...
        self.completion_decoder_2 = Fold(
            planes[1] + 3, step=stride[1] // 2, hidden_dim=256
        )
        self.completion_decoder_3 = Fold(
            planes[2] + 3, step=stride[2] // 2, hidden_dim=256
        )
        self.completion_decoder_4 = Fold(
            planes[3] + 3, step=stride[3] // 2, hidden_dim=256
        )
        self.completion_decoder_5 = Fold(
            planes[4] + 3, step=stride[4] // 2, hidden_dim=256
        )
    ...
    def update_moving_average(self):
        update_moving_average(
            self.target_ema_updater, self.target_encoder, self.PT_model
        )
