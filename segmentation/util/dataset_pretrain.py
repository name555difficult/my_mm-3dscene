...
class DatasetPretrain(Dataset):
    def __init__(
        ...
        if self.pretrain_dataset == "scannet":
            ...
            split_filenames = os.path.join(
                data_path, "meta_data", "scannetv2_{}.txt".format("train")
            )
            ...
            for scan in self.scan_names:
                if not os.path.exists("/dev/shm/{}".format(scan)):
                    ...
                    data = np.concatenate(
                        (
                            vert,
                            np.expand_dims(sem, axis=-1),
                            np.expand_dims(ins, axis=-1),
                        ),
                        axis=-1,
                    )  # npy, n*8
    ...
    def scene_crop(self, org_coord, org_feat, org_label, GT_feat):
        ...
        crop_ids = np.where(
            (org_coord[:, 0] < center_x + CROP_X)
            & (org_coord[:, 0] > center_x - CROP_X)
            & (org_coord[:, 1] < center_y + CROP_Y)
            & (org_coord[:, 1] > center_y - CROP_Y)
        )[0]
        t[crop_ids] += 1
    ...
    def __getitem__(self, idx):
        ...
        org_coord, org_feat, org_label = (
            org_data[:, :3],
            org_data[:, 3:6],
            org_data[:, 6],
        )
        org_coord, org_feat, org_label, GT_feat = data_prepare(
            org_coord,
            org_feat,
            org_label,
            self.split,
        ...
        org_coord, org_feat, org_label, GT_feat = self.scene_crop(
            org_coord, org_feat, org_label, GT_feat
        )
...
if __name__ == "__main__":
    ...
    for i, (coord, feat, target, target_coor, offset) in enumerate(
        train_loader
    ):  # (n, 3), (n, c), (n), (b)
        print(coord.size())
