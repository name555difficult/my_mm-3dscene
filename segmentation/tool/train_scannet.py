...
from util.data_util import collate_fn, collate_fn_s3dis
...
def get_parser():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="config/scannet/scannet_pointtransformer_repro.yaml",
        help="config file",
    )
    parser.add_argument(
        "opts",
        help="see config/scannets/cannet_pointtransformer_repro.yaml for all options",
        default=None,
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    scheduler = lr_scheduler.CosineAnnealingLR(
        optimizer, args.epochs, eta_min=0.0001, last_epoch=-1, verbose=False
    )
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (coord, feat, target, offset) in enumerate(
        ...
        # if args.final_tune:
        #     output, target = model([coord, feat, offset], target, args.final_tune)
        # else:
