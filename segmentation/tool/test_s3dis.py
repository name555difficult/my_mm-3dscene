...
def data_prepare():
    if args.data_name == "s3dis":
        ...
        data_list = [
            item[:-4] for item in data_list if "Area_{}".format(args.test_area) in item
        ]
    else:
...
def test(model, criterion, names):
    ...
    for idx, item in enumerate(data_list):
        ...
        pred_save_path = os.path.join(
            args.save_folder, "{}_{}_pred.npy".format(item, args.epoch)
        )
        label_save_path = os.path.join(
            args.save_folder, "{}_{}_label.npy".format(item, args.epoch)
        )
        if os.path.isfile(pred_save_path) and os.path.isfile(label_save_path):
            ...
            for i in range(idx_size):
                ...
                if args.voxel_max and coord_part.shape[0] > args.voxel_max:
                    coord_p, idx_uni, cnt = (
                        np.random.rand(coord_part.shape[0]) * 1e-3,
                        np.array([]),
                        0,
                    )
            ...
            for i in range(batch_num):
                ...
                coord_part = torch.FloatTensor(np.concatenate(coord_part)).cuda(
                    non_blocking=True
                )
                feat_part = torch.FloatTensor(np.concatenate(feat_part)).cuda(
                    non_blocking=True
                )
                offset_part = torch.IntTensor(np.cumsum(offset_part)).cuda(
                    non_blocking=True
                )
                with torch.no_grad():
                    pred_part = model(
                        [coord_part, feat_part, offset_part], training=False
                    )  # (n, k)
            ...
            loss = criterion(
                pred, torch.LongTensor(label).cuda(non_blocking=True)
            )  # for reference
