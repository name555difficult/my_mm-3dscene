...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    from model.pointtransformer.pointtransformer_pretrain import (
    ...
    optimizer = torch.optim.AdamW(
        model.parameters(), lr=args.base_lr, weight_decay=0.0005
    )
    ...
    train_data = DatasetPretrain(
        split="train",
        pretrain_dataset=args.data_name,
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (scene_coor, scene_rgb, scene_offsets) in enumerate(
        train_loader
    ):  # (n, 3), (n, c), (n)
        data_time.update(time.time() - end)
        scene_coor, scene_rgb = scene_coor.cuda(non_blocking=True), scene_rgb.cuda(
            non_blocking=True
        )
        scene_offsets = scene_offsets.cuda(non_blocking=True)
        c1, c2, c3, c4, c5, gt_pxo, ct_1, ct_2, ct_3, ct_4, ct_5 = model(
            [scene_coor, scene_rgb, scene_offsets]
        )
