...
def data_viz(data_dir, dump_dir=os.path.join(BASE_DIR, "data_viz_dump")):
    ...
    for idx in range(len(sunrgbd)):
        ...
        for i in range(uv.shape[0]):
            ...
            cv2.circle(
                img,
                (int(np.round(uv[i, 0])), int(np.round(uv[i, 1]))),
                2,
                color=tuple(color),
                thickness=-1,
            )
        ...
        if len(oriented_boxes) > 0:
            ...
            pc_util.write_oriented_bbox(
                oriented_boxes, os.path.join(dump_dir, "obbs.ply")
            )
        else:
            print("-" * 30)
            continue
        ...
        print("-" * 30)
        print(
            "Point clouds and bounding boxes saved to PLY files under %s" % (dump_dir)
        )
...
def extract_sunrgbd_data(
    ...
    for data_idx in data_idx_list:
        ...
        if skip_empty_scene and (
            len(objects) == 0
            or len([obj for obj in objects if obj.classname in type_whitelist]) == 0
        ):
            continue
            ...
            for obj in objects:
                ...
                try:
                    ...
                    box3d_pts_3d = sunrgbd_utils.my_compute_box_3d(
                        obj.centroid, np.array([obj.l, obj.w, obj.h]), obj.heading_angle
                    )
                    ...
                    sparse_inds = indices[
                        inds
                    ]  # turn dense True,False inds to sparse number-wise inds
                    for i in range(len(sparse_inds)):
                        j = sparse_inds[i]
                        point_votes[
                            j,
                            int(point_vote_idx[j] * 3 + 1) : int(
                                (point_vote_idx[j] + 1) * 3 + 1
                            ),
                        ] = votes[i, :]
...
def get_box3d_dim_statistics(
    idx_filename, type_whitelist=DEFAULT_TYPE_WHITELIST, save_path=None
):
