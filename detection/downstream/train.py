...
parser.add_argument(
    "--pretrained_path", default=None, help="Model pretrained path [default: None]"
)
parser.add_argument(
...
parser.add_argument(
    "--num_point", type=int, default=40000, help="Point Number [default: 20000]"
)
parser.add_argument(
    "--num_target", type=int, default=256, help="Proposal number [default: 256]"
)
parser.add_argument(
    "--vote_factor", type=int, default=1, help="Vote factor [default: 1]"
)
parser.add_argument(
    "--cluster_sampling",
    default="vote_fps",
...
parser.add_argument(
    "--ap_iou_thresh", type=float, default=0.25, help="AP IoU threshold [default: 0.25]"
)
parser.add_argument(
    "--max_epoch", type=int, default=500, help="Epoch to run [default: 180]"
)
parser.add_argument(
    "--batch_size", type=int, default=6, help="Batch Size during training [default: 8]"
)
parser.add_argument(
    "--learning_rate",
    type=float,
...
def load_backbone(model, weights):
    model_state = model.state_dict()
    filtered_weights = {
        k: v
        for k, v in weights.items()
        if k.startswith("backbone_net.") and v.size() == model_state[k].size()
    }
    finetune_unique_weights = {
        k: v
        for k, v in model_state.items()
        if not k.startswith("backbone_net.") or v.size() != weights[k].size()
    }
    return filtered_weights, finetune_unique_weights


# Load pretrained weights
if PRETRAINED_PATH is not None and os.path.isfile(PRETRAINED_PATH):
    log_string("-> loading pretrained weight '{}'".format(PRETRAINED_PATH))
    state = torch.load(
        PRETRAINED_PATH, map_location=lambda s, l: default_restore_location(s, "cpu")
    )["model_state_dict"]
    matched_weights, finetune_unique_weights = load_backbone(net, state)
    model_dict = net.state_dict()
    # print("before", net.state_dict()['backbone_net.fp2.mlp.layer1.bn.bn.bias'])
    model_dict.update(matched_weights)
    net.load_state_dict(model_dict)
    # print("after", net.state_dict()['backbone_net.fp2.mlp.layer1.bn.bn.bias'])
    log_string("-> loaded pretrained weight '{}'".format(PRETRAINED_PATH))
else:
    log_string("-> no pretrained weight found at '{}'".format(PRETRAINED_PATH))
...
def evaluate_one_epoch():
    ...
    ap_calculator = APCalculator(
        ap_iou_thresh=FLAGS.ap_iou_thresh, class2type_map=DATASET_CONFIG.class2type
    )
...
def train(start_epoch):
    ...
    for epoch in range(start_epoch, MAX_EPOCH):
        ...
        np.random.seed()
        train_one_epoch()
        # if EPOCH_CNT == 0 or EPOCH_CNT % 200 == 9: # Eval every 10 epochs
