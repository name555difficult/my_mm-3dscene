...
def mask_lowlevel_Diff(pxo, feat, mask_rate):
...
def mask_fuc(pc, mask_rate):
    ...
    mask_pxo, gt_pxo = mask_lowlevel_Diff([xyz, rgb, off], f, mask_rate)
...
class VoteNet(nn.Module):
    ...
    def update_moving_average(self):
        ...
        update_moving_average(
            self.target_ema_updater, self.target_encoder, self.backbone_net
        )
    ...
    def forward(self, inputs, mask_rate):
        ...
        mask_xyz, mask_feat, gt_xyz, gt_feat = mask_fuc(
            inputs["point_clouds"], mask_rate
        )

        end_points = self.backbone_net(
            torch.cat([mask_xyz, mask_feat], dim=-1), end_points
        )
        ...
        # exit(0)
        completion_pred = (
            self.expansion(end_points["fp2_features"]).permute(0, 2, 1).contiguous()
        )
        ...
        with torch.no_grad():
            ...
            end_points_gt = target_encoder(
                torch.cat([gt_xyz, gt_feat], dim=-1), end_points_gt
            )
        ...
        return (
            ...
            gt_xyz,
            gt_feat,
            contrast_part_feats.permute(0, 2, 1).contiguous(),
            end_points["fp2_features"].permute(0, 2, 1).contiguous(),
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        TRAIN_DATASET = SunrgbdDetectionVotesDataset(
            "train", num_points=20000, use_v1=True
        )

        # Model forward pass
        sample = TRAIN_DATASET[5]
        inputs = {
            "point_clouds": torch.from_numpy(sample["point_clouds"]).unsqueeze(0).cuda()
        }
    except:
