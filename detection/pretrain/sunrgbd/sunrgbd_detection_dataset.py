...
class SunrgbdDetectionVotesDataset(Dataset):
    def __init__(
        ...
        DATA_DIR = "/home/mutian/votenet/downstream/"
        assert num_points <= 50000
        self.use_v1 = use_v1
        if use_v1:
            self.data_path = os.path.join(
                DATA_DIR, "sunrgbd/sunrgbd_pc_bbox_votes_50k_v1_%s" % (split_set)
            )
        else:
            self.data_path = os.path.join(
                DATA_DIR, "sunrgbd/sunrgbd_pc_bbox_votes_50k_v2_%s" % (split_set)
            )

        self.raw_data_path = os.path.join(DATA_DIR, "sunrgbd/sunrgbd_trainval")
        self.scan_names = sorted(
            list(set([os.path.basename(x)[0:6] for x in os.listdir(self.data_path)]))
        )
    ...
    def __getitem__(self, idx):
        ...
        point_cloud = np.load(os.path.join(self.data_path, scan_name) + "_pc.npz")[
            "pc"
        ]  # Nx6
        bboxes = np.load(os.path.join(self.data_path, scan_name) + "_bbox.npy")  # K,8
        point_votes = np.load(os.path.join(self.data_path, scan_name) + "_votes.npz")[
            "point_votes"
        ]  # Nx10
        ...
        if self.augment:
            ...
            point_votes_end[:, 1:4] = np.dot(
                point_cloud[:, 0:3] + point_votes[:, 1:4], np.transpose(rot_mat)
            )
            point_votes_end[:, 4:7] = np.dot(
                point_cloud[:, 0:3] + point_votes[:, 4:7], np.transpose(rot_mat)
            )
            point_votes_end[:, 7:10] = np.dot(
                point_cloud[:, 0:3] + point_votes[:, 7:10], np.transpose(rot_mat)
            )
...
def viz_obb(
    pc, label, mask, angle_classes, angle_residuals, size_classes, size_residuals
):
...
def get_sem_cls_statistics():
    ...
    d = SunrgbdDetectionVotesDataset(
        use_height=True, use_color=True, use_v1=True, augment=True
    )
    sem_cls_cnt = {}
...
if __name__ == "__main__":
    ...
    viz_obb(
        sample["point_clouds"],
        sample["center_label"],
        sample["box_label_mask"],
        sample["heading_class_label"],
        sample["heading_residual_label"],
